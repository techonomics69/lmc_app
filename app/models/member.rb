...
class Member < ApplicationRecord
    ...
    CSV.generate(options) do |csv|
      ...
      all.each do |member|
        if member.membership
          values = member.membership.attributes.slice(*membership_atts).values_at(*membership_atts)
        end
        values += member.attributes.values_at(*member_atts)
        csv << values
      end
  ...
  def self.to_csv_membership(member_atts = %w[first_name last_name], membership_atts = DEFAULT_MEMBERSHIP_ATTS, options = {})
    CSV.generate(options) do |csv|
      ...
      all.each do |member|
        ...
        if member.membership
          values += member.membership.attributes.slice('created_at').values_at('created_at')
        end
        csv << values
      end
    end
  ...
  def self.to_csv_emergency_contact(member_atts = %w[first_name last_name], em_contact_atts = DEFAULT_EM_CONT_ATTS, options = {})
    CSV.generate(options) do |csv|
      ...
      all.each do |member|
        ...
        if member.emergency_contact
          values += member.emergency_contact.attributes.slice(*em_contact_atts).values_at(*em_contact_atts)
        end
        csv << values
      end
    end
  ...
  DEFAULT_MEMBER_ATTS = %w[first_name
                           last_name
                           email
                           home_phone
                           mob_phone
  ...
  DEFAULT_MEMBERSHIP_ATTS = %w[bmc_number
                               membership_type
                               subs_paid
                               fees_received_on
                               made_full_member
  ...
  DEFAULT_EM_CONT_ATTS = %w[name
                            ...
                            country
                            primary_phone
                            secondary_phone].freeze

  BMC_MEMBER_ATTS = %w[title
                       first_name
                       last_name
                       dob
                       address_1
  ...
  BMC_MEMBERSHIP_ATTS = ['bmc_number'].freeze
